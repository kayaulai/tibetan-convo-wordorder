...
```{r warning=FALSE}
model_2order_ridge = ROLLing(df, featsChosen, id = "verbID", order = "argOrder", formulaType = "quad", reg = "ridge", theta = 1^power_opt)

get_quad_ridge_aic = function(theta){
  model_curr = ROLLing(df, featsChosen, id = "verbID", order = "argOrder",
                   formulaType = "quad",
                   reg = "ridge", theta = theta,
                   iter.max = 100)
  AIC(model_curr)
}

aics_grid = sapply(seq(-10, 10, 1), function(x) get_quad_ridge_aic(10^x))
power_opt = seq(-10, 10, 1)[which.min(aics_grid)]
model_interact_ridge_opt = ROLLing(df, featsChosen, id = "verbID", order = "argOrder", formulaType = "quad", reg = "ridge", theta = 10^power_opt)
coefLollipops(model_interact_ridge_opt, top = 30)
```

```{r}
model_genlasso_010 = ROLLing_genlasso(
  coxDF = coxDF,
  model_mle = model_mle,
  hyper = c(lambda_lasso = 1,
            lambda_ridge = 10^-10,
...
predict(model_genlasso_010)$by_clause$correct %>% mean
```

```{r}

#This produces error, fix later
genlasso_models = ROLLing_genlasso_path(coxDF,
    model_mle,
    lambdas_lasso = 1,
    lambdas_ridge = 10 ^ seq(-10, 10, 5), 
    lambdas_genlasso = 1,
    hyper_genlasso = list(corThreshold = .3),
    splitVec = splitVec, cv = TRUE,
    elnetMode = "all")
